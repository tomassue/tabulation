            ...
            <tr>
                ...
                <td style="vertical-align: top;" width="85%">
                    <div style="text-align: center;padding-top:20px;padding-bottom:20px;">
                        <div style="font-size: 15pt;"><i>Ranking Sheet</i></div>
                    ...
                    @foreach ($participants as $item)
                        @php
                            ...
                            foreach ($judges as $judge) {
                                ...
                                if ($ranked) {
                                    $part["judge-{$judge->user_id}"] = $ranked;
                                    $sum += $ranked;
                                } else {
                                    $part["judge-{$judge->user_id}"] = 0;
                                }
                                $part["subtotal-{$judge->user_id}"] = $item->getHigalaayScoreByJudge($judge->id, $category->category);
                            }
                            $part['grand'] = $sum;
                            $part['participant'] = $item->participant;
                            //get the participant
                            $participant = $item->participant;

                            //add to array
                            array_push($grands, $part);

                            // After collecting all participants, sort by grand total to get rankings
                            usort($grands, function ($a, $b) {
                                // If 'a' is zero and 'b' is not, 'a' comes after (positive return value).
                                if ($a['grand'] == 0 && $b['grand'] != 0) {
                                    return 1;
                                }
                                // If 'b' is zero and 'a' is not, 'b' comes after (negative return value).
                                if ($b['grand'] == 0 && $a['grand'] != 0) {
                                    return -1;
                                }
                                // If both are zero or neither are zero, use normal ascending order comparison.
                                // The spaceship operator (<=>) is useful here for a concise comparison.
                                return $a['grand'] <=> $b['grand'];

                                // return $b['grand'] <=> $a['grand']; // Descending order (Lowest first)
                            });
                    ...
                    <table class="table bordered">
                        <thead>
                            <tr>
                                ...
                                @endforeach
                                <th width="10%" style="font-size: 9pt;">TOTAL RANK</th>
                                <th width="10%" style="font-size: 9pt;">FINAL RANK</th>
                            </tr>
                        </thead>
                        <tbody>
                            @forelse ($grands as $item)
                                <tr>
                                    ...
                                    @foreach ($judges as $judge)
                                        <td class="text-center">{{ $item["subtotal-{$judge->user_id}"] }}</td>
                                        <td class="text-center" style="font-weight: bold">{{ $item["judge-{$judge->user_id}"] != 0 ? bong_ordinal($item["judge-{$judge->user_id}"]) : '-' }}</td>
                                    @endforeach
